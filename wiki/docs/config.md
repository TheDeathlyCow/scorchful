...
* :star: Heat from the Sun `heatFromSun`: How much environmental heating to apply from the Sun every tick
...
* :star: Humid biome sweating efficiency `humidBiomeSweatEfficiency`: A multiplier for `temperatureFromWetness` when in a humid biome (Jungles, Rivers, Oceans)
...
* Fear detection range multiplier `veryProtectiveArmorHeatResistance`: How much to multiply an entity's (including players) hostile mob detection range by.
...
Options for integration with Seasons mods like [Fabric Seasons](https://modrinth.com/mod/fabric-seasons) and [Serene Seasons](https://modrinth.com/mod/serene-seasons). Note that [Thermoo Patches](https://modrinth.com/mod/thermoo-patches) is required for Scorchful to be aware of seasons.

* :star: Enable seasons integration `enableSeasonsIntegration`: Toggles seasons integration. If set to false, Scorchful will treat all biomes as if it were Spring.
